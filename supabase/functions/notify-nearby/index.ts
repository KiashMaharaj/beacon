...
Deno.serve(async (req: Request) => {
  ...
  const rpc = await fetch(`${supabaseUrl}/rest/v1/rpc/nearby_push_targets`, {
    method: 'POST',
    headers: {
      apikey: serviceKey,
  ...
  const tokens: string[] = Array.isArray(rows)
    ? rows.map((r: any) => r.push_token).filter(Boolean)
    : [];
  ...
  await Promise.all(
    tokens.map(async (token) => {
      ...
      if (res.ok) {
        ...
        // Stale/invalid token: clear it so we stop trying.
        await fetch(
